...
from stream.badwords import badwords
...
class PostDebate(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('stream:debate_detail', args=[self.debate_category, self.pk, self.slug])
...
class Attachment(models.Model):
    ...
    file = VersatileImageField(
        ...
        upload_to='attachments/stream', help_text="Add Pictures"
    )

    #file = models.FileField(upload_to='attachments/stream', help_text="Attach images to post. 2 max")
    def __str__(self):
...
class Badge(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('stream:see_badge', args=[self.name,])
    def __str__(self):
        """
